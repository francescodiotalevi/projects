...
static long auxdisp_ioctl(struct file *filp, unsigned int cmd,
			  unsigned long param);
...
static ssize_t black_screen(void)
{
	...
	unsigned int length =
	    NUM_CHIPS_PER_ROW * 16 * d_support.bpp_format * NUM_ROWS *
	    LINES_PER_ROW;
	...
	d_support.tx_dma_desc =
	    dmaengine_prep_slave_single(d_support.tx_dma_chan,
					d_support.tx_dma_buf_phys, length,
	...
	// Enable the IP
	set_register_bits(ctrl_reg,
			  AUXDISP_CTRL_IE | (AUXDISP_CTRL_CMD_MSK &
					     AUXDISP_CTRL_CMD_WRTGS), 0xFF);
	...
	d_support.tx_cookie =
	    d_support.tx_dma_desc->tx_submit(d_support.tx_dma_desc);
	...
	time_left =
	    wait_for_completion_timeout(&d_support.tx_cmp, d_support.tx_tmo);
	status =
	    dma_async_is_tx_complete(d_support.tx_dma_chan, d_support.tx_cookie,
				     NULL, NULL);
	if (time_left == 0) {
		printk(KERN_ALERT MSG_PREFIX "DMA timed out\n");
	} else if (status != DMA_COMPLETE) {
		printk(KERN_ALERT MSG_PREFIX
		       "Tx got completion callback, but status is \'%s\'\n",
...
static ssize_t chardev_write(struct file *fp, const char __user * buf,
			     size_t length, loff_t * off)
{
	...
	for (i = 0; i < 32; i++) {
		...
		offset =
		    (i % 2) ? ((i >> 1) +
			       16) * NUM_CHIPS_PER_ROW * 16 *
		    d_support.bpp_format : (i >> 1) * NUM_CHIPS_PER_ROW * 16 *
		    d_support.bpp_format;
		//printk("bpp:%d %d %d %d\n", d_support.bpp_format, offset, i, (i%2) ? ((i>>1)+16) : (i>>1));
		if (copy_from_user
		    (d_support.tx_dma_buf_virt +
		     i * NUM_CHIPS_PER_ROW * 16 * d_support.bpp_format,
		     buf + offset,
		     NUM_CHIPS_PER_ROW * 16 * d_support.bpp_format)) {
			printk(KERN_ALERT MSG_PREFIX
			       "Error in copy_from_user of the image data\n");
			return -1;
		}
	}
...
int register_chardev(struct platform_device *devp)
{
	...
	if ((res =
	     alloc_chrdev_region(&d_support.devt, 0, 1, RET_CLASS_NAME)) < 0) {
		printk(KERN_ALERT MSG_PREFIX
	...
	if ((device_create
	     (ret_class, NULL, d_support.devt, NULL,
	      RET_CTL_DEVNAME)) == NULL) {
	...
	if ((res = cdev_add(&d_support.chdev, d_support.devt, 1)) < 0) {
		printk(KERN_ALERT MSG_PREFIX "Error adding device "
		       RET_CTL_DEVNAME "\n");
...
static int init_FC_registers(uint48_t ** FC)
{
	...
	for (r = 0; r < d_support.impl_numofrows; r++) {
		memcpy(tmp_ptr, (unsigned char *)(&FC[r][0]),
		       sizeof(uint48_t) * d_support.impl_numofchips);
		tmp_ptr =
		    tmp_ptr + sizeof(uint48_t) * d_support.impl_numofchips;
	...
	d_support.tx_dma_desc =
	    dmaengine_prep_slave_single(d_support.tx_dma_chan,
					d_support.tx_dma_buf_phys,
					d_support.impl_numofrows *
					(d_support.impl_numofchips *
					 sizeof(uint48_t) +
					 ((d_support.
					   impl_numofchips & 0x1) ?
					  sizeof(dummy) : 0)), DMA_MEM_TO_DEV,
					flags);
	...
	// Enable the IP
	set_register_bits(ctrl_reg,
			  AUXDISP_CTRL_EN | AUXDISP_CTRL_IE |
			  (AUXDISP_CTRL_CMD_MSK & AUXDISP_CTRL_CMD_WRTFC),
			  0xFF);
...
static int init_fc(void)
{
	...
	FC = (uint48_t **) kmalloc(d_support.impl_numofrows *
				   sizeof(uint48_t *), GFP_KERNEL);
	for (r = 0; r < d_support.impl_numofrows; r++)
		FC[r] =
		    (uint48_t *) kmalloc(d_support.impl_numofchips *
					 sizeof(uint48_t), GFP_KERNEL);
...
static int auxdisp_probe(struct platform_device *devp)
{
	...
	if (!res) {
		printk(KERN_INFO MSG_PREFIX
		       "Error getting platform device memory\n");
		return -1;
	}
	...
	printk(KERN_INFO MSG_PREFIX "cfg (%d.%d.%d)\n",
	       d_support.impl_numofrows, d_support.impl_numofchips,
	       d_support.impl_numlines);
	if ((d_support.impl_numlines != LINES_PER_ROW)
	    || (d_support.impl_numofchips != NUM_CHIPS_PER_ROW)
	    || (d_support.impl_numofrows != NUM_ROWS)) {
		printk(KERN_ALERT MSG_PREFIX
		       "Error in kernel module configuration and auxdisp implementation\n");
		return -1;
	}
	// This is the size of a FC
	input_picsize =
	    NUM_ROWS * LINES_PER_ROW * NUM_CHIPS_PER_ROW * 16 *
	    sizeof(uint48_t);
	// Allocate DMA coherent memory
	d_support.tx_dma_buf_virt =
	    (unsigned char *)dma_alloc_coherent(&(devp->dev),
						PAGE_ALIGN(input_picsize),
						&(d_support.tx_dma_buf_phys),
						GFP_KERNEL);
	if (!d_support.tx_dma_buf_virt) {
		printk(KERN_INFO MSG_PREFIX
		       "Error allocating tx dma coherent device memory\n");
		return -1;
	}
	...
	result =
	    request_irq(d_support.irq, irq_handler, IRQF_SHARED, "int_auxdisp",
			&d_support);
	if (result) {
...
static int auxdisp_remove(struct platform_device *devp)
{
	...
	dma_free_coherent(&(devp->dev), PAGE_ALIGN(input_picsize),
			  d_support.tx_dma_buf_virt, d_support.tx_dma_buf_phys);
...
//    unsigned int bpp;
	...
	case IOC_GET_VER:
		...
		if (copy_to_user
		    ((unsigned int *)param, &tmp, sizeof(unsigned int)))
			goto ctuser_err;
	...
	case IOC_SET_FC:
		...
		for (r = 0; r < d_support.impl_numofrows; r++)
			if (copy_from_user
			    (&(FC[r][0]), (&((uint48_t **) param)[r][0]),
			     d_support.impl_numofchips * sizeof(uint48_t)))
				goto cfuser_err;
	...
	case IOC_GEN_REG:
		if (copy_from_user
		    (&temp_reg, (struct gla_regs *)param, sizeof(temp_reg)))
			goto cfuser_err;
		...
		if (temp_reg.rw == 0) {
			...
			if (copy_to_user
			    ((struct gla_regs *)param, &temp_reg,
			     sizeof(temp_reg)))
				goto cfuser_err;
		} else {
